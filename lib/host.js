...
export function apply(ctx) {
  ...
  // 可视化配置页 RPC（client → host）
  const harness = ctx.get('harness')
  if (harness) {
    harness.handle('telegram-bridge:get-config', () => {
      const c = readConfig()
      return { enabled: c.enabled, tokenSet: !!c.botToken, allowedUserIds: c.allowedUserIds,
        workdir: c.workdir, agentCmd: c.agentCmd, pollSeconds: c.pollSeconds,
        running: !!handle, pid: handle ? handle.pid : null, configFile }
    })
    harness.handle('telegram-bridge:set-config', (args) => {
    ...
    harness.handle('telegram-bridge:status', () => ({ running: !!handle, pid: handle ? handle.pid : null }))
  }

// Host only: reads a config file and manages the Python Telegram bridge subprocess.
// No client/settings-namespace dependency (no browser page needed).
import z from '@deepseek-ai/schemastery'
import fs from 'node:fs'
import path from 'node:path'

export const name = 'telegram-bridge-manager'
export const inject = ['subprocess', 'timer']

// 插件自身配置（可在 cordis.patch.yml 的 config 里覆盖）
export const config = z.object({
  bridgeDir: z.string().default('D:/dsh-telegram-full'),
  pyCommand: z.string().default('pythonw'),
  // 留空则用 <bridgeDir>/bridge.config.json
  configFile: z.string().default(''),
})

export function apply(ctx, config) {
  const sub = ctx.subprocess
  const configPath = config.configFile || path.join(config.bridgeDir, 'bridge.config.json')
  let handle = null

  const DEFAULTS = {
    enabled: false,
    botToken: '',
    allowedUserIds: '',
    workdir: config.bridgeDir,
    agentCmd: 'npx --yes @deepseek-ai/dsh --profile headless',
    pollSeconds: 30,
  }

  function readConfig() {
    try {
      const c = JSON.parse(fs.readFileSync(configPath, 'utf8'))
      return {
        enabled: !!c.enabled,
        botToken: (c.botToken || '').trim(),
        allowedUserIds: (c.allowedUserIds || '').trim(),
        workdir: (c.workdir || '').trim() || config.bridgeDir,
        agentCmd: (c.agentCmd || '').trim() || DEFAULTS.agentCmd,
        pollSeconds: c.pollSeconds || 30,
      }
    } catch (e) {
      console.error('[telegram-bridge] config read failed (using defaults):', String(e), 'file:', configPath)
      return DEFAULTS
    }
  }

  async function start() {
    const c = readConfig()
    let py = config.pyCommand
    try { py = await sub.resolveExecutable(config.pyCommand) } catch {}
    handle = sub.spawn({
      argv: [py, 'bot.py'],
      cwd: config.bridgeDir,
      stdio: {
        stdin: 'ignore',
        stdout: { maxBytes: 1024 * 1024, spill: { maxBytes: 4 * 1024 * 1024 } },
        stderr: { maxBytes: 1024 * 1024, spill: { maxBytes: 4 * 1024 * 1024 } },
      },
      graceMs: 2000,
      env: {
        TELEGRAM_BOT_TOKEN: c.botToken,
        ALLOWED_USER_IDS: c.allowedUserIds,
        AGENT_WORKDIR: c.workdir,
        AGENT_CMD: c.agentCmd,
      },
    })
    console.log('[telegram-bridge] started PID', handle.pid)
    handle.done
      .then((out) => console.log('[telegram-bridge] exited', out.exitCode, out.signal))
      .catch(() => {})
  }

  function stop() {
    if (handle) {
      try { handle.terminate() } catch {}
      handle = null
    }
  }

  function sync() {
    const c = readConfig()
    if (c.enabled && c.botToken) {
      if (!handle) start().catch((e) => console.error('[telegram-bridge] start error', String(e)))
    } else if (handle) {
      stop()
    }
  }

  sync()
  // 每 5s 重读配置，改 bridge.config.json 即自动启停/重启
  ctx.interval(() => sync(), 5000)

  // dispose：终止桥
  ctx.effect(() => () => {
    try { if (handle) handle.terminate() } catch {}
  })
}
